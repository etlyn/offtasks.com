...
## September 12 Device-first Update

Current inventory: **23 features / 117 scenarios**: mobile 12 / 58, web 11 / 59.
Eight device-first cases were added in `mobile/features/guest-onboarding.feature`;
auth and overview expectations now reflect optional accounts and four tabs.
The `mobile/maestro/guest.yaml` native flow is authored but **not executed past
driver startup**: Maestro 2.10.0's XCTest runner was killed before connecting
on iOS 26.5. Real-account integration was blocked by Docker failure.
Shared runner tests: 13 passed. This does not establish app-action coverage.
See `offtasks-mobile/docs/guest-first-readiness.md` in the app repository for
the executed test matrix, fixed issues, known gaps and release blockers.

The May inventory below is retained as historical context. Its old Today/
Tomorrow tabs, mandatory-login and device-only account notes descriptions are
...
`etlyn/offtasks-mobile`. The original scenario inventory is unchanged.
...
| Mobile   | `App.tsx`, `src/navigation/*`, `src/features/*`, `src/components/*`, `src/providers/*`, `src/lib/*`, `src/hooks/*`, `src/utils/*` in `etlyn/offtasks-mobile` |
...
- `src/features/settings/Settings.screen.tsx` exists in the mobile repository, but the active `AppNavigator` exposes Dashboard and Statistics through the drawer. Mobile settings/account coverage is therefore written against the active drawer account controls.
...
- Generated cucumber JSON/HTML execution reports are intentionally not stored as the coverage source of truth. Run a platform-specific command from the `etlyn-e2e` root to generate fresh execution output under `reports/`.
...
- `yarn e2e:test:all` (includes ControlX)
