      ...
      <div className="space-y-3">
        {completedTasks.length === 0 ? (
          <EmptyState
            ...
            description="Complete a task and it will appear here with its completion date."
            className="py-20"
          />
        ) : (
                  ...
                  {(task.priority || task.label) && (
                    <div className="flex flex-wrap items-center gap-[6px]">
                      ...
                      {task.label &&
                        (() => {
                          const config =
                            getCategoryConfig(task.label) ||
                            getDefaultCategoryColor();
                          return (
                            <span
                              className={`inline-flex items-center px-[8px] py-[3px] rounded-[6px] border font-['Poppins',_sans-serif] text-[11px] opacity-70 ${config.bgColor} ${config.borderColor} ${config.color}`}
                            >
                              {task.label}
                            </span>
                          );
